...
* Using the above data, I want to predict $\texttt{Price}$ ($Y$ here) using  $\texttt{Interest Rate}$ as $x$.
...
* **Goal**: Find values of $\beta_0$, $\beta_1$, and $\sigma^2$ that  minimizes the last expression $\sum_{i=1}^n \epsilon_i^2$.
...
* To minimize $\sum_{i=1}^n \epsilon_i^2$, we take derivatives of the expression with respect to $\beta_0$ and $\beta_1$ and equal it to $0$. We get the following two <span style="color:blue">normal equations:
...
data <- data.frame(Income, Expenditure)

# Make predictions using the model
data$Prediction <- predict(model)

# Calculate residuals (errors)
data$Residual <- data$Expenditure - data$Prediction
...
ggplot(data, aes(x = Income, y = Expenditure)) +
  ...
  geom_point(aes(y = Prediction), color = "green", size = 3, shape = 4) +
  
  # Add the residual lines (errors)
  geom_segment(aes(xend = Income, yend = Prediction), linetype = "dotted", color = "gray") +
  
  # Add labels and title
  labs(title = "Income vs Food Expenditure Regression",
       subtitle = "With Regression Line, Predictions, and Errors",
       x = "Income (in $1000s)",
       y = "Expenditure (in $1000s)") +
  
  # Customize legend
  scale_color_manual(name = "Legend",
                     values = c("Actual" = "blue", "Predicted" = "green", "Regression Line" = "red")) +
...
\begin{align*}
\widehat{\sigma}^2 &= \frac{1}{n-2} e_i^2 \\
&= \frac{1}{n-2} \big(y_i - \widehat{y}_i \big)^2 \\
&= \frac{1}{n-2} \big(y_i - \widehat{\beta}_0 - x_i \widehat{\beta}_1 \big)^2 \\
\end{align*}
