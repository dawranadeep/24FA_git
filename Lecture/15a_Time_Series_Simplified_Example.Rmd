---
title: "Time Series with R: Simplified Version"
...
* Additionally, we can inspect the **ACF** (Autocorrelation Function) and **PACF** (Partial Autocorrelation Function) plots to help identify any remaining dependencies:
  ...
  - **ACF plot:**
    - Autocorrelation is defined as the correlation between observations of a time series separated by a lag, $\delta$; i.e., <span style = "color:purple">$\rho_h$ = cor$\big[y_t, y_{t-\delta}\big]$.</span>
    - Helps us identify the number of MA terms needed in the model.
  - **PACF plot:** 
    - Partial autocorrelation is the autocorrelation after you remove the effect of past few time points; i.e., <span style = "color:purple">$\phi_h$ = cor$\big[y_t - \hat{y}_t, y_{t-\delta} - \hat{y}_{t-\delta}\big]$.</span>
    - Helps identify the number of AR terms.
...
```{r}
par(mfrow = c(1, 2))
acf(nybirths_ts, lag.max = 48, main = "ACF plot", ylab = "Original birth data")
pacf(nybirths_ts, lag.max = 48,  main = "PACF plot", ylab = "Original birth data")
```
...
* <span style = "color: #BA4A00">**What do you see in the ACF plot?**</span>
  ...
  - <span style = "color:limegreen">**Here, see that autocorrelation is quite high at small lags, then it is decreasing. Then again, when it is the same month repeating over the next year, the autocorrelation is going higher and higher.**</span>
...
* <span style = "color: #BA4A00">**What do you see in the PACF plot?**</span>
  ...
  -  <span style = "color:limegreen">**Here, see that partial autocorrelation is quite high at the beginning, then it is decreasing and becoming negative since there may have been an over-adjustment from the initial lags. Some bumps are visible around the same months over the next years. **</span>
...
## Step 9: Automatic Modeling using $\texttt{auto.arima}$

* $\texttt{R}$ has an inbuilt function, called $\texttt{auto.arima}$, that automatically finds some working values that works for your data.
* You don't manually need to find the number of AR and MA terms; $\texttt{auto.arima}$ finds those values for you.

```{r}
automated_birthmodel = auto.arima(nybirths_ts)
automated_birthmodel
```


This is a seasonal ARIMA model. It has two components:

1. Non-seasonal component -- ARIMA(0,1,3):

  * This part is the non-seasonal component of the model.
  * The middle coefficient $1$ means that the series has been differnced once (i.e., $y_t - y_{t-1}$) before fitting the AR and MA parts.
  * Then, $0$ AR and $3$ MA terms have been used in this component.
  
2. Seasonal component -- (2,1,1)[12]:

  * To model the seasonal part, it found a seasonality of 12, which corresponds to monthly data.
  * Again, the series got differenced once before fitting the model. 
  * Finally, $2$ seasonal AR terms and $1$ seasonal moving average (MA) term has been used to fit this part of the data.


<br><br><br> 


## Step 10: Residual Test
...
```{r}
autoplot(residuals2, main = "Residual plot over time")
```
